...
import { create_topic, get_user_from_email } from '@/lib/database';
...
export default async function handler(
  ...
  if (session) {
    ...
    const func = await create_topic(name, username)
    if (typeof func == "object"){
      res.status(200).json({ message: 'Created!', error: false, data: func })
    } else {
